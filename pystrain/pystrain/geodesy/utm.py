...
def utm2ell(E, N, zone, ell=Ellipsoid("wgs84"), lcm=None):
    ...
    while abs(dlat) > 1e-12:
        A0 = 1 - (e2 / 4) - (e22 * 3 / 64.0) - (e23 * 5 / 256.0) - (e24 * 175 /
                                                                    16384.0)
        A2 = (3 / 8.0) * (e2 + (e22 / 4.0) + (e23 * 15 / 128.0) -
                          (e24 * 455 / 4096.0))
        A4 = (15 / 256.0) * (e22 + (e23 * 3 / 4.0) - (e24 * 77 / 128.0))
        A6 = (35 / 3072.0) * (e23 - (e24 * 41 / 32.0))
        A8 = -(315 / 131072.0) * e24
        f1 = a * (A0 * lat1 - A2 * sin(2 * lat1) + A4 * sin(4 * lat1) -
                  A6 * sin(6 * lat1) + A8 * sin(8 * lat1)) - N / ko
        f2 = a * (A0 - 2 * A2 * cos(2 * lat1) + 4 * A4 * cos(4 * lat1) -
                  6 * A6 * cos(6 * lat1) + 8 * A8 * cos(8 * lat1))
        dlat = -f1 / f2
        lat1 = lat1 + dlat
    RN = ell.N(lat1)
    RM = ell.M(lat1)
    ...
    E3 = pow(E0, 5) / 120. * (5 + 6 * h2 + 28 * t2 - 3 * h22 + 8 * t2 * h2 +
                              24 * t4 - 4 * h23 + 4 * t2 * h22 + 24 * t2 * h23)
    E4 = pow(E0, 7) / 5040. * (61 + 662 * t2 + 1320 * t4 + 720 * t6)
    lon = (1 / cos(lat1)) * (E1 - E2 + E3 - E4) + lcm

    E0 = E / ko
    N1 = (t * pow(E0, 2)) / (2 * RM * RN)
    N2 = (t * pow(E0, 4)) / (24 * RM * pow(RN, 3)) * (5 + 3. * t2 + h2 -
                                                      4 * h22 - 9 * h2 * t2)
    N3 = (t * pow(E0, 6)) / (720 * RM * pow(RN, 5)) * (
        61 - 90 * t2 + 46 * h2 + 45 * t4 - 252 * t2 * h2 - 5 * h22 + 100 * h23 -
        66 * t2 * h22 - 90 * t4 * h2 + 88 * h24 + 225 * t4 * h22 +
        84 * t2 * h23 - 192 * t2 * h24)
    N4 = (t * pow(E0, 8)) / (40320 * RM * pow(RN, 7)) * (1385 + 3633 * t2 +
                                                         4095 * t4 + 1575 * t6)
...
def ell2utm(lat, lon, ell=Ellipsoid("wgs84"), zone=None, lcm=None):
    ...
    E3 = pow(lam, 5) * pow(
        coslat, 5) / 120 * (5 - 18 * t2 + t4 + 14 * h2 - 58 * t2 * h2 +
                            13 * h22 + 4 * h23 - 64 * t2 * h22 - 24 * t2 * h23)
    E4 = pow(lam, 7) * pow(coslat,
                           7) / 5040 * (61 - 479 * t2 + 179 * t4 - t4 * t2)
    E = Eo + ko * RN * (E1 + E2 + E3 + E4)

    N1 = S / RN
    N2 = pow(lam, 2) / 2 * sinlat * coslat
    N3 = pow(lam, 4) / 24 * sinlat * pow(coslat,
                                         3) * (5 - t2 + 9 * h2 + 4 * h22)
    N4 = pow(lam, 6) / 720 * sinlat * pow(coslat, 5) * (
        61 - 58 * t2 + t4 + 270 * h2 - 330 * t2 * h2 + 445 * h22 + 324 * h23 -
        680 * t2 * h22 + 88 * h24 - 600 * t2 * h23 - 192 * t2 * h24)
    N5 = pow(lam, 8) / 40320 * sinlat * pow(
        coslat, 7) * (1385 - 311 * t2 + 543 * t4 - t6)
...
if __name__ == "__main__":
    ...
    lats = [
        0.6808606982218858, -0.2102493430122449, -0.6599718220321278,
        0.6758316289450003
    ]
    # lons = [22.969694, -77.0428, 144.9631, -9.1393]
    lons = [
        0.40089679623260527, -1.3446505249554874, 2.530083388897792,
        -0.15951087632751776
    ]
    # ellipsoid grs80
    ell = Ellipsoid('grs80')
    # octave results
    octave = [[4319781.22749958, 670541.267192695, 34, 0.366519142918809],
    ...
    for i in range(0, len(lats)):
        print('> Testing Point #{} lat={} lon={}'.format(
            i, degrees(lats[i]), degrees(lons[i])))
        n, e, z, l = ell2utm(lats[i], lons[i], ell)
        print('\tNorthing={} Easting={} Zone={} Central Mer.={}'.format(
            n, e, z, l))
        print('\tOctave (abs) diffs: dN{} dE{} dZ{} dCM{}'.format(
            abs(n - octave[i][0]), abs(e - octave[i][1]), abs(z - octave[i][2]),
            abs(l - octave[i][3])))
        ...
        if abs(clat - lats[i]) > 5e-10 or abs(clon - lons[i]) > 5e-10:
            ...
            print('\tdlat={} dlon={} in decimal degrees'.format(
                degrees(abs(clat - lats[i])), degrees(abs(clon - lons[i]))))
            print('\tdLat={} dLon={} in seconds'.format(
                degrees(abs(clat - lats[i])) * 3600e0,
                degrees(abs(clon - lons[i])) * 3600e0))
            print('\tInput {}, {} output {}, {}'.format(degrees(lats[i]),
                                                        degrees(lons[i]),
                                                        degrees(clat),
                                                        degrees(clon)))
