...
def write_station_info(sta_lst, filename='station_info.dat'):
    ...
    with open(filename, 'w') as fout:
        ...
        for idx, sta in enumerate(sta_lst):
            print(
                '{:10s} {:+10.5f} {:10.5f} {:+7.2f} {:+7.2f} {:+7.3f} {:+7.3f}'.
                format(sta.name, degrees(sta.lon), degrees(sta.lat),
                       sta.ve * 1e03, sta.vn * 1e03, sta.se * 1e03,
                       sta.sn * 1e03),
                file=fout)
    return
...
def compute__(grd, sta_list_utm, utmzone, fout, fstats, vprint_fun, **dargs):
    ...
    for x, y in grd:
        ...
        assert ZN == utmzone
        vprint_fun(
            '[DEBUG] Grid point at {:+8.4f}, {:8.4f} or E={:}, N={:}'.format(
                x, y, E, N))
            ...
            try:
                ...
                if fstats:
                    print('{:+9.4f} {:+10.4f} {:6d} {:14.2f} {:10.2f} {:12.3f}'.
                          format(x, y, len(sstr.__stalst__),
                                 sstr.__options__['d_coef'],
                ...
                vprint_fun(
                    '[DEBUG] Too few observations to estimate strain at {:+8.4f}, {:8.4f}. Point skipped.'
                    .format(x, y))
            except ArithmeticError:
                vprint_fun(
                    '[DEBUG] Failed to compute parameter VcV matrix for strain at {:+8.4f}, {:8.4f}. Point skipped'
                    .format(x, y))
        else:
            vprint_fun(
                '[DEBUG] Skipping computation at {:+8.4f},{:8.4f} because of limited coverage (max_beta= {:6.2f}deg.)'
                .format(x, y, degrees(max(sstr.beta_angles()))))
        node_nr += 1
    print('[DEBUG] Estimated Strain Tensors for {} out of {} nodes'.format(
        nodes_estim, node_nr))
    fout.close()
    if fstats:
        fstats.close()
...
parser.add_argument(
    ...
    required=False,
    help=
    'The x-axis grid step size in degrees. This option is only relevant if the program computes more than one strain tensors.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'The y-axis grid step size in degrees. This option is only relevant if the program computes more than one strain tensors.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'Choose a method for strain estimation. If \'shen\' is passed in, the estimation will follow the algorithm described in Shen et al, 2015, using a weighted least squares approach. If \'veis\' is passed in, then the region is going to be split into delaneuy triangles and a strain estimated in each barycenter.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'Only relevant for \'--mehod=shen\'. Choose between a \'gaussian\' or a \'quadratic\' spatial weighting function.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the lower limit for searching for an optimal D-parameter value. Unit is km.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the upper limit for searching for an optimal d-param value. Unit is km.'
)
...
parser.add_argument(
    ...
    required=False,
    help=
    'Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the step size for searching for an optimal d-param value. Unit is km.'
)
...
if __name__ == '__main__':
    ...
    if args.multiproc_mode:
        if args.method == 'shen':
            ...
            print(
                "[DEBUG] Using multithreaded version; available CPU's: {:02d}".
                format(cpu_count))
        else:
            print(
...
## import dill module for windows multithreading processing
    ...
    if fstats:
        print_model_info(fstats, sys.argv, dargs)
    ...
    if not os.path.isfile(args.gps_file):
        print('[ERROR] Cannot find input file \'{}\'.'.format(args.gps_file),
              file=sys.stderr)
        sys.exit(1)
    try:
        ...
        print(err)
        print('[ERROR] Failed to parse input file: \"{:}\"'.format(
            args.gps_file))
        sys.exit(1)
    ...
    if 'region' in args:
        ...
        try:
            lonmin, lonmax, latmin, latmax = [
                float(i) for i in args.region.split('/')
            ]
            if args.cut_outoflim_sta or args.method == 'veis':
                ...
                sta_list_ell = cut_rectangle(lonmin, lonmax, latmin, latmax,
                                             sta_list_ell, True)
                Npst = len(sta_list_ell)
                vprint(
                    '[DEBUG] Stations filtered to fit input region: {:7.3f}/{:7.3f}/{:7.3f}/{:7.3f}'
                    .format(lonmin, lonmax, latmin, latmax))
                vprint(
                    '[DEBUG] {:4d} out of original {:4d} stations remain to be processed.'
                    .format(Npst, Napr))
                if Npst < 3:
                    print(
                        '[DEBUG] Left with only {:d} stations! Cannot do anything'
                        .format(Npst))
                    trigger_exit = True
        except:
    ...
    if 'region' in args and not args.method == 'veis' and not args.cut_outoflim_sta:
        vprint(
            '[DEBUG] Filtering stations based on their distance from region barycentre.'
        )
        Napr = len(sta_list_ell)
        mean_lon, mean_lat = radians(lonmin + (lonmax - lonmin) /
                                     2e0), radians(latmin +
                                                   (latmax - latmin) / 2e0)
        bc = Station(lon=mean_lon, lat=mean_lat)
        endpt = Station(lon=radians(lonmax), lat=radians(latmax))
        cutoffdis = abs(endpt.haversine_distance(bc) /
                        1e3)  # barycentre to endpoint (km)
        ...
        sta_list_ell = [
            s for s in sta_list_ell
            if s.haversine_distance(bc) / 1e3 <= cutoffdis
        ]
        Npst = len(sta_list_ell)
        print(
            '[DEBUG] {:4d} out of original {:4d} stations remain to be processed.'
    ...
    vprint('[DEBUG] Mean longtitude is {} deg.; using Zone = {} for UTM'.format(
        mean_lon, utm_zone))
    sta_list_utm = deepcopy(sta_list_ell)
    for idx, sta in enumerate(sta_list_utm):
        N, E, Zone, lcm = ell2utm(sta.lat, sta.lon, Ellipsoid("wgs84"),
                                  utm_zone)
    ...
    print(
        '{:^9s} {:^9s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s}'
        .format('Latitude', 'Longtitude', 'vx+dvx', 'vy+dvy', 'w+dw',
                'exx+dexx', 'exy+dexy', 'eyy+deyy', 'emax+demax', 'emin+demin',
                'shr+dshr', 'azi+dazi', 'dilat+ddilat',
                'sec. invariant+dsec inv.'),
        file=fout)
    print(
        '{:^9s} {:^9s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s} {:^15s}'
        .format('deg', 'deg', 'mm/yr', 'mm/yr', 'deg/Myr', 'nstrain/yr',
    ...
    if args.one_tensor:
        ...
        print('[DEBUG] Total running time: {:10.2f} sec.'.format(
    ...
    if args.method == 'shen':  ## Going for Shen algorithm ...
        ...
        ##+ station list.
        if 'region' in args:
            grd = pystrain.grid.Grid(lonmin, lonmax, args.x_grid_step, latmin,
        ...
        if args.multiproc_mode:
            ...
            p1 = multiprocessing.Process(target=compute__,
                                         args=(grd1, sta_list_utm, utm_zone,
                                               fout1, fstats1, vprint),
                                         kwargs=dargs)
            p2 = multiprocessing.Process(target=compute__,
                                         args=(grd2, sta_list_utm, utm_zone,
                                               fout2, fstats2, vprint),
                                         kwargs=dargs)
            p3 = multiprocessing.Process(target=compute__,
                                         args=(grd3, sta_list_utm, utm_zone,
                                               fout3, fstats3, vprint),
                                         kwargs=dargs)
            p4 = multiprocessing.Process(target=compute__,
                                         args=(grd4, sta_list_utm, utm_zone,
                                               fout4, fstats4, vprint),
                                         kwargs=dargs)
            ...
            compute__(grd, sta_list_utm, utm_zone, fout, fstats, vprint,
                      **dargs)
    else:
        ...
        print(
            '[DEBUG] Estimating Strain Tensors at the barycentre of Delaunay triangles'
        )
        dlnout = open('delaunay_info.dat', 'w')
        points = numpy.array([[sta.lon, sta.lat] for sta in sta_list_utm])
        tri = Delaunay(points)
        print('[DEBUG] Number of Delaunay triangles: {}'.format(
            len(tri.simplices)))
        for idx, trng in enumerate(tri.simplices):
            print('[DEBUG] {:5d}/{:7d}'.format(idx + 1, len(tri.simplices)),
                  end="\r")
            ## triangle barycentre
            cx = (sta_list_utm[trng[0]].lon + sta_list_utm[trng[1]].lon +
                  sta_list_utm[trng[2]].lon) / 3e0
            cy = (sta_list_utm[trng[0]].lat + sta_list_utm[trng[1]].lat +
                  sta_list_utm[trng[2]].lat) / 3e0
            ##  Construct a strain instance, at the triangle's barycentre, with only
            ##+ 3 points (in UTM) and equal_weights weighting scheme.
            sstr = ShenStrain(cx,
                              cy, [
                                  sta_list_utm[trng[0]], sta_list_utm[trng[1]],
                                  sta_list_utm[trng[2]]
                              ],
                              weighting_function='equal_weights')
            sstr.estimate()
            ...
            print('> {:}, {:}, {:}'.format(sta_list_utm[trng[0]].name,
                                           sta_list_utm[trng[1]].name,
                                           sta_list_utm[trng[2]].name),
                  file=dlnout)
            print(
                '{:+8.5f} {:8.5f}\n{:+8.5f} {:8.5f}\n{:+8.5f} {:8.5f}\n{:+8.5f} {:8.5f}'
                .format(*[
                    degrees(x) for x in [
                        sta_list_ell[trng[0]].lon, sta_list_ell[trng[0]].lat,
                        sta_list_ell[trng[1]].lon, sta_list_ell[trng[1]].lat,
                        sta_list_ell[trng[2]].lon, sta_list_ell[trng[2]].lat,
                        sta_list_ell[trng[0]].lon, sta_list_ell[trng[0]].lat
                    ]
                ]),
                file=dlnout)
